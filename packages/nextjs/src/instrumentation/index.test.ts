...
describe('browsonicInstrumentation', () => {
  ...
  // The default `reportError` is `console.error` — Browsonic is a
  // browser-only error tracker by design, so the SDK does not ship
  // a server-runtime ingest path. Consumers who want one supply
  // their own `reportError` callback. A previous revision shipped
  // a fetch-POST bridge here; it was reverted because it pulled the
  // SDK's scope toward Sentry-weight server observability.
  it('default reportError prints to console.error (no fetch bridge)', () => {
    ...
    try {
      ...
      void onRequestError(new Error('x'), { path: '/x' }, { routerKind: 'App Router' });

      expect(fetchSpy).not.toHaveBeenCalled();
      expect(consoleSpy).toHaveBeenCalledTimes(1);
      expect(consoleSpy.mock.calls[0]![0]).toBe('[browsonic]');
    } finally {
