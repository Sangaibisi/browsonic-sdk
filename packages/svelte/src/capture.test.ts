...
/**
 * Capture-wrapper regression suite. The wrappers are thin shims that
 * resolve the SDK from `window` and forward — tests lock in the
 * defensive isolation contract: throws inside the SDK never bubble
 * out, missing SDK never throws.
 */
...
describe('captureMessage', () => {
  it('forwards message + level to the resolved SDK', () => {
    const sdk = installFakeSdk();
  ...
  it('defaults level to "info"', () => {
    ...
    expect(sdk.captureMessage).toHaveBeenCalledWith('hello', 'info');
  });
...
describe('addBreadcrumb', () => {
  it('forwards a breadcrumb to the resolved SDK', () => {
    const sdk = installFakeSdk();
