...
### Sprint 6 — Vue + Svelte Adapters (P2, 2 hafta) — DURUM: KAPANDI 2026-05-04

#### Pre-flight Check

- [x] **2026-05-04** Protokol 1.1 tüm adımları geçildi.
  - [x] (1.1.1) `browsonic-sdk` AGENTS.md (root) S5.5 sonrası genişletilmiş haliyle taze. Monorepo discipline + `mkdir packages/<framework>` workflow uygulandı.
  - [x] (1.1.2) SPRINT_PLAN.md tam okundu — S5.5 KAPANDI, S8 KAPANDI, S6 plan kalemleri (`@browsonic/vue` + `@browsonic/svelte`) tutarlı.
  - [x] (1.1.3) CROSS_REPO_IMPACTS.md okundu — S6 öncesi 5 pending entry (S2 service ×2, S5 dashboard, S5 landing, S8 service ×3 + dashboard).
  - [x] (1.1.4) AGENTS.md ↔ S6 çatışması yok. Adapter pattern S5'te onaylandı; ADAPTER_TEMPLATE.md monorepo-flow ile yeniden yazılı.
  - [x] (1.1.5) Working tree temiz — S8 closure (`aac0032`) push edildi.
...
- [2026-05-04] **S6 milestone 1**: `@browsonic/vue` 0.1 — `packages/vue/` workspace bootstrap + tüm yüzey + 24 test — durum: ✅
  - Commit/PR: bkz. milestone 1 commit hash `b83c1bf`
  - Test/CI: typecheck clean, lint 0/0 (test ergonomics için `unbound-method` + `no-unnecessary-type-assertion` test files'da off; `_*` unused vars ignore pattern eklendi). Test **24/24 passed** (composables × 11, error-boundary × 7, plugin × 6). Build × 3 (esm/cjs/types).
  - Notlar: Pure-TS implementation — `.vue` SFC YOK, `defineComponent` + render function ile boundary. `errorCaptured` `false` döndürüyor (boundary autoritatif olarak hatayı yutuyor; ancestor `errorHandler`'a iletilmiyor — istenirse user `onError`'da kendi handler'ı ile relay edebiliyor). `useUser` hem plain hem `Ref` kabul ediyor (`watch` ile değişiklikte reapply, immediate). Plugin `app.config.errorHandler`'ı **chain ediyor**, override etmiyor (önceki handler `try/catch` ile çağrılıyor — pinia devtools / başka plugin'lerle uyumlu). `chainErrorHandler: false` flag escape hatch olarak duruyor.
- [2026-05-04] **S6 milestone 2**: `@browsonic/svelte` 0.1 — `packages/svelte/` workspace bootstrap + tüm yüzey + 25 test — durum: ✅
  - Commit/PR: bkz. milestone 2 commit hash `3c2a9df`
  - Test/CI: typecheck clean, lint 0/0, test **25/25 passed** (handle-error × 8, user-store × 8, capture × 9). Build × 3 (esm/cjs/types). `.svelte` dosyası yok → Svelte compiler build chain'de yok.
  - Notlar: **Bilinçli divergence** — `<BrowsonicErrorBoundary>` shipped EDİLMEDİ. Gerekçe: Svelte 5 `<svelte:boundary>` native primitive (framework feature), Svelte 4'te ise temiz boundary alternatifi yok. Yarım çalışan shim yerine README + AGENTS.md "Divergences from ADAPTER_TEMPLATE" altında doc edildi; consumer Svelte 5 native boundary'nin `onerror`'ından `captureError` çağırıyor. Yüzey: `handleErrorWithBrowsonic` (SvelteKit `handleError` factory, optional chain), `subscribeUser` (Svelte readable-store → SDK user bridge, store-shape agnostic), `captureError` / `captureMessage` / `addBreadcrumb` (window-singleton lookup'lı standalone wrappers), `resolveSdk` (lower-level lookup). Peer: `svelte ^4.0.0 || ^5.0.0`.

#### Sprint Sonu Cross-Repo Etki Kontrolü

- [x] **2026-05-04** Post-flight (1.3) tüm adımları geçildi.
  - (1.3.1) İş Logu M1 + M2 kayıtlarını içeriyor.
  - (1.3.2) Etkilenen repolar: **browsonic-landing-astro** — Vue + Svelte adapter CTA'ları + npm rozetleri (opsiyonel; CROSS_REPO entry #10/11). **browsonic-service** + **browsonic-dashboard** etkisi yok (adapter'lar SDK API'sini tüketiyor; wire format değişmedi).
  - (1.3.3) CROSS_REPO_IMPACTS.md güncellendi — 2 yeni opsiyonel landing satırı + S6 lesson-learned.
  - (1.3.4) typecheck/lint/test 3 paket için yeşil (sdk + vue + svelte). Build çalışıyor.
  - (1.3.5) Conventional Commits — `feat(vue):` (M1) + `feat(svelte):` (M2). Stamp commit'leri opsiyonel: hash'ler iş logunda inline.
  - (1.3.6) S6 başlığı KAPANDI 2026-05-04, Sprint Özet Tablosu satırı güncellendi.
...
- [2026-05-04] **S8 milestone 3**: `withScope(fn)` transient scope public API + `Scope` public interface + sync/async overloads + try/finally + Promise.finally restore semantics + 11 yeni unit test (scope.test.ts) — durum: ✅
  - Commit/PR: bkz. milestone 3 commit hash `d91fd88`
