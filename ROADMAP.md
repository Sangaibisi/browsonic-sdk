...
- _No active work — **the original 19-week plan is complete** (2026-05-04) and the **0.3 follow-up sweep is complete** (2026-05-05). Five adapters reached 0.3 surface this round (vue / svelte / angular / astro / remix); next.js sits at 0.2 with the 0.3 instrumentation helper shipped. Future work is the **source-map pipeline** (design draft now in [`docs/design/SOURCEMAP_PIPELINE.md`](./docs/design/SOURCEMAP_PIPELINE.md), waiting on a design review pass before the implementation sprint), the parking-lot items per per-package ROADMAPs, and any community-driven additions._

## Recently shipped

### 0.3 sweep (2026-05-04 → 2026-05-05) — adapter-level features

- **`@browsonic/vue` 0.3** — `installRouterInstrumentation({ includeIntent })` adds a `beforeEach` "intent" breadcrumb alongside the existing `afterEach` channel, `installPiniaIntegration` stamps Pinia store-action errors with `setContext('pinia', { storeId, action, args, errorMessage, state? })`, the boundary now ships an explicit Composition + Options API parity test suite, and a Suspense + async setup test pins the contract that the boundary catches errors from `<script setup async>` and top-level `await` in setup.
- **`@browsonic/svelte` 0.3** — `withBrowsonicAction` wraps SvelteKit `actions: {}` handlers so unhandled throws are reported then re-thrown (so SvelteKit's failure response stays intact); `reportErrorPage` is a one-shot, idempotent capture for `+error.svelte`'s `<script>` block with reference-keyed `WeakSet` de-dupe.
- **`@browsonic/angular` 0.3** — `createBrowsonicHttpReporter` ships a `(request, error) => void` callback consumers wire into a 5-line `HttpInterceptor` class (no `@angular/common/http` / `rxjs` runtime dep). New `@browsonic/angular/decorated` sub-entry adds `BrowsonicDecoratedService` (`@Injectable({ providedIn: 'root' })`) and `provideBrowsonicUserSignal` (`WritableSignal<UserContext | null>` bridged to `sdk.setUser` via Angular's `effect`) for consumers who want runtime `@angular/core` ergonomics — the default entry stays type-only / peer-only.
- **`@browsonic/astro` 0.3** — `withBrowsonicAstroAction` mirrors `withBrowsonicRouteHandler` for Astro Actions; `tagAsAstroIsland(name)` stamps `astro.island` on the SDK's active scope so per-framework boundaries inside the island inherit the tag automatically; `renderContentCollectionMeta` + the after-swap reader wire Content Collection identity into the navigation breadcrumb (`breadcrumb.data.contentCollection`) via a build-time meta-tag convention.
- **`@browsonic/remix` 0.3** — `useRemixNavigationBreadcrumbs(useNavigation(), useMatches())` emits a `category: 'navigation'` breadcrumb on each non-`idle` → `'idle'` transition with `routeId` (leaf) + `routeChain` (parent → leaf joined with ›) so cross-shell URL collisions become triagable.
- **`@browsonic/nextjs` 0.3 (partial)** — `@browsonic/nextjs/instrumentation` sub-entry ships `browsonicInstrumentation()` factory returning the `{ register, onRequestError }` pair Next.js's `instrumentation.ts` file convention expects. Today it validates config + forwards to `console.error`; the wire-up is forward-compatible so future server-runtime capture lands without consumer code change. App-Router `BrowsonicErrorPage` accepts optional `pathname` / `params` props that land as `nextjs.pathname` tag + `nextjs.params` context. Pages-Router companions (`browsonicPagesAppInit` / `browsonicPagesErrorInitialProps`) shipped in the 0.2 sweep. Build-time sourcemap upload via `withBrowsonicConfig` remains deferred until the source-map pipeline lands.

### Tooling / hygiene (2026-05-04 → 2026-05-05)

- **All 7 dependabot alerts cleared** (6× high `@angular/core` XSS in i18n + SVG attributes, 1× moderate `picomatch` ReDoS) via dev-dep + npm `overrides` sweep — `@angular/core` dev dep bumped to `^19`, `@semantic-release/npm` to `^13`, root `overrides` pin `picomatch ^4.0.4` and force a nested `npm ^11` inside `semantic-release`'s bundle. `npm audit` reports 0 vulnerabilities.
- **Documentation sweep** — 5 stale "Status: 0.1 surface" README banners refreshed across vue / svelte / nextjs / astro / remix to reflect the actually-shipped surface; 6 adapter `AGENTS.md` files refreshed alongside; root README's package table replaced "planned, S6/S7/S10" markers with real npm-version badges + per-adapter surface descriptions; cross-repo impact log gained an "Adapter 0.3 sweep" historical-note entry summarising four lessons-learned (reporter-factory pattern, hook-as-callback, re-throw discipline, partial-close ROADMAP markers).
- **Source-map pipeline design draft** — [`docs/design/SOURCEMAP_PIPELINE.md`](./docs/design/SOURCEMAP_PIPELINE.md) captures the architecture (build-time CLI + bundler plugins → service ingest → lazy dashboard symbolication), seven open design questions (token scope, release format, retention, inline vs external, sourcesContent embedding, etc.), cross-repo coordination preview, and a ~16-day effort estimate so the reopen sprint lands implementation without re-deriving the design.
- **`@browsonic/cli` 0.1 — `upload-sourcemaps` (with `--dry-run`)** — first concrete piece of the sourcemap pipeline lands ahead of the service endpoint. CLI walks a dist tree, finds every `.map` file, and POSTs each via multipart form-data to `<baseUrl>/v1/sourcemaps`. `--dry-run` mode runs the discovery + payload-size flow without making any HTTP request — useful for CI smoke tests before the ingest endpoint is live, and forward-compatible (the command shape doesn't change when the service ships). Bearer-auth via `BROWSONIC_SOURCEMAP_TOKEN` env var so CI never leaks tokens onto shell history. Pure-TypeScript, zero npm runtime deps (Node 20+ built-in `fetch` + `node:util.parseArgs`). 45 tests across walker / args / upload / command-orchestration. Programmatic API exposed for the future `@browsonic/build-tools` bundler-plugin package.

## Older shipped (2026-04-29 → 2026-05-04)
